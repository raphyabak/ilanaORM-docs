...
// Exclude a range
const nonMinors = await User.query()
...
// Posts created on the 15th of any month (all databases)
const midMonthPosts = await Post.query()
  .whereDay('created_at', 15)
  .get();

// Posts created after noon (all databases)
const afternoonPosts = await Post.query()
  .whereTime('created_at', '>', '12:00:00')
  .get();
```

<Note>`whereDate`, `whereDay`, `whereMonth`, `whereYear`, and `whereTime` all work across MySQL, PostgreSQL, and SQLite.</Note>
...
// Page helper — equivalent to offset((page-1) * perPage).limit(perPage)
const users = await User.query()
  .forPage(3, 15) // page 3, 15 per page
  .get();
```
...
```javascript
// Add each user's latest post title as a column
const users = await User.query()
  .addSelect({
    latest_post_title: (sub) => sub
      .from('posts')
      .select('title')
...
users[0].latest_post_title; // "My latest article"
```
...
```javascript
// Order users by the number of posts they have
const users = await User.query()
  .orderBySubquery((sub) => {
    sub.from('posts').count('id').whereRaw('posts.user_id = users.id');
  }, 'desc')
  .get();
```
...
// Creating through the scope automatically sets status = 'published'
const post = await Post.query().published().new({ title: 'Hello World' });
// post.status === 'published'  (set by the scope, not the caller)
await post.save();
```
...
```javascript
const post = await Post.query().published().create({ title: 'Hello World' });
...
// Increment views without fetching the post
...
```javascript
// Returns plain objects instead of model instances
const users = await User.query()
...
console.log(users[0].name); // 'John' — plain object, no ORM overhead
```
...
```javascript
// Throws ModelNotFoundException if no match, Error if multiple matches
const config = await AppConfig.query()
  .where('key', 'site_title')
  .sole();

// Good for unique constraints — catches duplicates early
const user = await User.query()
  .where('email', req.body.email)
  .sole(); // alerts you if email uniqueness is broken
```
...
const users = await User.query()
  .where('is_active', true)
  .tap((query) => console.log('SQL:', query.toSql()))
  .orderBy('name')
  .get();

// Or inspect mid-chain during development
const results = await Post.query()
  .published()
  .tap((q) => console.log('Before pagination:', q.toSql()))
  .paginate(1, 10);
```
...
### Relation existence (recommended)
...
const authors = await User.query().has('posts').get();

// Users who have more than 5 posts
const prolific = await User.query().has('posts', '>', 5).get();

// Users who have at least one published post (with a constraint)
const publishedAuthors = await User.query()
...
const lurkers = await User.query().doesntHave('posts').get();
// Also available as:
const lurkers2 = await User.query().whereDoesntHave('posts').get();
...
```javascript
// Manual WHERE EXISTS (use when you need full control over the subquery)
