...
```javascript
// images table has: id, url, imageable_type, imageable_id
...
class Post extends Model {
  image() {
    return this.morphOne('Image', 'imageable'); // stores imageable_type='Post'
  }
  static { this.register(); }
}
...
class User extends Model {
  avatar() {
    return this.morphOne('Image', 'imageable'); // stores imageable_type='User'
  }
  static { this.register(); }
}

// Usage
const post = await Post.with('image').find(1);
...
posts[0].relations.comments; // Error: 'comments' was not eager loaded on Post

// ✅ Always eager load when strictLoading is on
const posts = await Post.with('comments').get();
posts[0].relations.comments; // works fine
```
...
await post.tags().attach(tagId, { created_by: 'admin' }); // with pivot attributes

// Detach
await post.tags().detach(tagId);
await post.tags().detach(); // detach all

// Sync — detach all current, then attach these
await post.tags().sync([1, 3, 5]);

// Sync with pivot attributes (object form: { id: pivotAttrs })
await post.tags().sync({
  1: { created_by: 'admin' },
  3: { created_by: 'editor' },
});

// Toggle — attach if not present, detach if already attached
await post.tags().toggle([1, 2, 3]);

// Update an existing pivot row without detach/reattach
await post.tags().updateExistingPivot(tagId, { created_by: 'moderator' });
```
...
// Saving a comment will also UPDATE posts SET updated_at = NOW() WHERE id = comment.post_id
await comment.save();
```
